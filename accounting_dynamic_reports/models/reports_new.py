...
class AccountCommonReportNew(models.TransientModel):
    _name = "new.accounting.report"
    ...
    def get_account_lines(self, data):
        ...
        for report in child_reports:
            ...
            if report.parent_id:
                ...
                # p_name = p_name.replace(" ", "-") + "-" + str(report.parent_id.id)
            else:
                p_name = False
            vals = {
                'r_id': report.id,
            ...
            if res[report.id].get('account'):
                ...
                for account_id, value\
                    ...
                    #  we add them to the lines with a level equals to their level in
                    ...
                    vals = {
                        'account': account.id,
                        'a_id': account.code + re.sub('[^0-9a-zA-Z]+', '', account.name) + str(account.id),
                    ...
                    if data['debit_credit']:
                        ...
                        if not account.company_id.currency_id.is_zero(vals['debit']) or\
                                not account.company_id.currency_id.is_zero(vals['credit']):
                            flag = True
    ...
    def view_report(self):
        ...
        data['form'] = self.read(
                ['date_from', 'enable_filter', 'debit_credit', 'date_to',
                 'account_report_id', 'target_move', 'view_format'])[0]
