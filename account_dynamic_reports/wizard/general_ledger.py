...
class InsGeneralLedger(models.TransientModel):
    ...
    def build_detailed_move_lines(self, offset=0, account=0, fetch_range=FETCH_RANGE):
        ...
        move_lines = []
        #######################################################################
        # To find the running balance of previous
        #######################################################################
        if data.get('initial_balance'):
            # initial Balance ---------------------------
            sql = ('''
        ...
        ###########################################################################
        # To find count of total matching record. For pagination purpose
        ###########################################################################
        sql = ('''
            SELECT COUNT(*)
            FROM account_move_line l
        ...
        count = cr.fetchone()[0]
        #####################################################################
        # To get basic details to plot report
        #####################################################################
        ...
        cr.execute(sql)
        for row in cr.dictfetchall():
            # Uncomment in case of running balance. Not tested
